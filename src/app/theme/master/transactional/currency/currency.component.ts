...
export class CurrencyComponent implements OnInit {
  ...
  loadData() {
    ...
    this.data = this.service.getAllCurrency(status);
  }
    ...
    if (item.status.indexOf('WAITING FOR') !== -1) {
      ...
      swal({
        ...
        if (result.value) {
          ...
          this.service.deleteCurrency(request).subscribe(data => {
  ...
  submit() {
    if (!this.request.currency) {
      this.toastService.warning('Currency must be filled');
    } else {
      this.spinner.isSpinnerVisible = true;
      if (this.request.id) {
        this.service.updateCurrency(this.request).subscribe(data => {
        ...
        this.service.addCurrency(this.request).subscribe(data => {
  ...
  approve(approved: boolean) {
    if (!approved) {
      ...
      this.service.approvalCurrency(data).subscribe(resp => {
  ...
  checkPrivilage(access): boolean {
    let privilage = this.localStorageService.retrieve('privilege');
    let roleMenu = privilage.menus.filter(x => x.alias_menu == 'trans_currency')[0];
