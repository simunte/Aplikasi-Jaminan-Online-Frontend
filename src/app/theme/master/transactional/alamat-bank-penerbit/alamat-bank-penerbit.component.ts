...
@Component({
  selector: 'app-alamat-bank-penerbit',
  templateUrl: './alamat-bank-penerbit.component.html',
  styleUrls: ['./alamat-bank-penerbit.component.scss',
    '../../../../../assets/icon/icofont/css/icofont.scss',
    '../../../../../../node_modules/sweetalert2/src/sweetalert2.scss']
})
export class AlamatBankPenerbitComponent implements OnInit {
  ...
  public moduleString = 'Alamat Bank Penerbit';
  ...
  loadData() {
    ...
    this.data = this.service.getAllAlamatBankPenerbit(status);
  }
    ...
    if (item.status.indexOf('WAITING FOR') !== -1) {
      ...
      swal({
        ...
        if (result.value) {
          ...
          this.service.deleteAlamatBankPenerbit(request).subscribe(data => {
  ...
  submit() {
    ...
    if (!this.request.code) {
      ...
      this.spinner.isSpinnerVisible = false;
    } else if (!this.request.cabang) {
      this.toastService.warning('Cabang must be filled');
      this.spinner.isSpinnerVisible = false;
    } else if (!this.request.alamat_bank_penerbit) {
      this.toastService.warning('Alamat must be filled');
      this.spinner.isSpinnerVisible = false;
    } else {
      if (this.request.id) {
        this.service.updateAlamatBankPenerbit(this.request).subscribe(data => {
        ...
        this.service.addAlamatBankPenerbit(this.request).subscribe(data => {
  ...
  approve(approved: boolean) {
    if (!approved) {
      ...
      this.service.approvalAlamatBankPenerbit(data).subscribe(resp => {
  ...
  checkPrivilage(access): boolean {
    let privilage = this.localStorageService.retrieve('privilege');
    let roleMenu = privilage.menus.filter(x => x.alias_menu == 'trans_alamat_bank_penerbit')[0];
