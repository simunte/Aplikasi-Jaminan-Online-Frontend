...
@Component({
  selector: 'app-unit-pengguna',
  templateUrl: './unit-pengguna.component.html',
  styleUrls: ['./unit-pengguna.component.scss',
    '../../../../../assets/icon/icofont/css/icofont.scss',
    '../../../../../../node_modules/sweetalert2/src/sweetalert2.scss']
})
export class UnitPenggunaComponent implements OnInit {
  ...
  public moduleString = 'Unit Pengguna';
  ...
  loadData() {
    ...
    this.data = this.service.getAllUnitPengguna(status);
  }
    ...
    if (item.status.indexOf('WAITING FOR') !== -1) {
      ...
      swal({
        ...
        if (result.value) {
          ...
          this.service.deleteUnitPengguna(request).subscribe(data => {
  ...
  submit() {
    this.spinner.isSpinnerVisible = true;
    if (!this.request.code_unit_pengguna) {
      this.toastService.warning('Kode must be filled');
      this.spinner.isSpinnerVisible = false;
    } else if (!this.request.unit_pengguna) {
      this.toastService.warning('Unit Pengguna must be filled');
      this.spinner.isSpinnerVisible = false;
    } else {
      if (this.request.id) {
        this.service.updateUnitPengguna(this.request).subscribe(data => {
        ...
        this.service.addUnitPengguna(this.request).subscribe(data => {
  ...
  approve(approved: boolean) {
    if (!approved) {
      ...
      this.service.approvalUnitPengguna(data).subscribe(resp => {
  ...
  checkPrivilage(access): boolean {
    let privilage = this.localStorageService.retrieve('privilege');
    let roleMenu = privilage.menus.filter(x => x.alias_menu == 'trans_unit_pengguna')[0];
