...
@Component({
  selector: 'app-jenis-jaminan',
  templateUrl: './jenis-jaminan.component.html',
  styleUrls: ['./jenis-jaminan.component.scss',
    '../../../../../assets/icon/icofont/css/icofont.scss',
    '../../../../../../node_modules/sweetalert2/src/sweetalert2.scss']
})
export class JenisJaminanComponent implements OnInit {
  ...
  public moduleString = 'Jenis Jaminan';
  ...
  loadData() {
    ...
    this.data = this.service.getAllJenisJaminan(status);
  }
    ...
    if (item.status.indexOf('WAITING FOR') !== -1) {
      ...
      swal({
        ...
        if (result.value) {
          ...
          this.service.deleteJenisJaminan(request).subscribe(data => {
  ...
  submit() {
    this.spinner.isSpinnerVisible = true;
    if (!this.request.code_jaminan) {
      this.toastService.warning('Kode must be filled');
      this.spinner.isSpinnerVisible = false;
    } else if (!this.request.jenis_jaminan) {
      this.toastService.warning('Jenis Jaminan must be filled');
      this.spinner.isSpinnerVisible = false;
    } else {
      if (this.request.id) {
        this.service.updateJenisJaminan(this.request).subscribe(data => {
        ...
        this.service.addJenisJaminan(this.request).subscribe(data => {
  ...
  approve(approved: boolean) {
    if (!approved) {
      ...
      this.service.approvalJenisJaminan(data).subscribe(resp => {
