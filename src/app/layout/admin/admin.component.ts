...
import { UiSwitchComponent } from 'ng2-ui-switch';
...
export class AdminComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.newDate = moment(new Date).format('MMMM Do YYYY, h:mm a')

    this.setBackgroundPattern('theme1');
    this.getMenuItems();
    this.dataRoles = this.localStorageService.retrieve('auth');
  ...
  getMenuItems() {
    var privilege = this.localStorageService.retrieve('privilege');
    var selectedRole = privilege.name;
    var GenerateMenu = [];
    GenerateMenu = privilege.menus;
    this.menuItemsGet = this.menuItems.getAll(GenerateMenu, selectedRole);
  }
  ...
  changeLanguage(event) {
    ...
    if(lang == 'id'){
      moment.locale('id')
      this.newDate = moment(new Date).format(' Do MMMM YYYY, h:mm a')
